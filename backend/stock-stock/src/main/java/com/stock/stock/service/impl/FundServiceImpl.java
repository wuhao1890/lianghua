...
public class FundServiceImpl implements FundService {
    ...
    public List<FundInfoDTO> getFundNavHistory(String code, int days) {
        ...
        try {
            // Use ProcessBuilder to call curl.exe (most reliable way for this API)
            // curl -s "http://api.fund.eastmoney.com/f10/lsjz?callback=jQuery&fundCode=000001&pageIndex=1&pageSize=30" -H "User-Agent: Mozilla/5.0" -H "Referer: https://fund.eastmoney.com"
            java.util.List<String> cmd = new java.util.ArrayList<>();
            cmd.add("C:\\WINDOWS\\System32\\curl.exe");
            cmd.add("-s");
            cmd.add("--max-time");
            cmd.add("10");
            cmd.add("http://api.fund.eastmoney.com/f10/lsjz?callback=jQuery&fundCode=" + code + "&pageIndex=1&pageSize=" + days);
            cmd.add("-H");
            cmd.add("User-Agent: Mozilla/5.0");
            cmd.add("-H");
            cmd.add("Referer: https://fund.eastmoney.com");
            ...
            while (pos < arrayContent.length()) {
                int bs = arrayContent.indexOf("{", pos);
                int be = arrayContent.indexOf("}", bs);
                if (bs < 0 || be < 0) break;

                String obj = arrayContent.substring(bs + 1, be);
                Map<String, String> fields = new HashMap<>();
                String[] pairs = obj.split(",");
                for (String pair : pairs) {
                    int ci = pair.indexOf(":");
                    if (ci > 0) {
                        fields.put(pair.substring(0, ci).replace("\"", "").trim(),
                                pair.substring(ci + 1).replace("\"", "").trim());
                    }
                }
                ...
                FundInfoDTO dto = new FundInfoDTO();
                dto.setCode(code);
                dto.setNavDate(fields.getOrDefault("FSRQ", "").replace("\"", ""));
                try {
                    String dwjz = fields.get("DWJZ");
                    if (dwjz != null && !dwjz.isEmpty()) {
                        dwjz = dwjz.replace("\"", "");
                        dto.setNav(new BigDecimal(dwjz));
                    }
                    String ljjz = fields.get("LJJZ");
                    if (ljjz != null && !ljjz.isEmpty()) {
                        ljjz = ljjz.replace("\"", "");
                        dto.setAccNav(new BigDecimal(ljjz));
                    }
                    if (dto.getNav() != null && !result.isEmpty()) {
                        BigDecimal prev = result.get(result.size() - 1).getNav();
                        if (prev != null && prev.compareTo(BigDecimal.ZERO) > 0) {
                            dto.setChangePercent(dto.getNav().subtract(prev)
                                    .divide(prev, 4, RoundingMode.HALF_UP)
                                    .multiply(new BigDecimal("100"))
                                    .setScale(2, RoundingMode.HALF_UP));
                        }
                    }
                    if (dto.getNavDate() != null && !dto.getNavDate().isEmpty()) {
                        result.add(dto);
                    }
                } catch (Exception ignored) {}
                pos = be + 1;
            ...
            log.info("获取基金净值历史: {} - {} 条", code, result.size());
        } catch (Exception e) {
            log.warn("获取基金净值历史失败 {}: {}", code, e.getMessage());
        }
        return result;
    }
}
