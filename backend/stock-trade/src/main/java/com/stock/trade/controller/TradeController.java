...
public class TradeController {
    ...
    public ResponseEntity<Map<String, Object>> placeOrder(
        ...
        Map<String, Object> result = new HashMap<>();
        try {
            TradeOrder order = tradeService.placeOrder(userId, request);
            result.put("code", 200);
    ...
    /**
     * 检查止损止盈是否触发
     * GET /api/trade/order/{id}/stop-check?currentPrice=xxx
     */
    @GetMapping("/order/{id}/stop-check")
    public ResponseEntity<Map<String, Object>> stopCheck(
            @RequestHeader("X-User-Id") Long userId,
            @PathVariable Long id,
            @RequestParam BigDecimal currentPrice) {
        Map<String, Object> result = new HashMap<>();
        try {
            List<TradeOrder> triggered = tradeService.checkStopConditions(userId, id, currentPrice);
            result.put("code", 200);
    ...
    public ResponseEntity<Map<String, Object>> getMode(
        ...
        Map<String, Object> result = new HashMap<>();
        try {
            String mode = tradeService.getTradeMode(userId);
            result.put("code", 200);
            result.put("message", "查询成功");
    ...
    /**
     * 切换交易模式
     * POST /api/trade/mode?mode=PAPER
     */
    @PostMapping("/mode")
    public ResponseEntity<Map<String, Object>> switchMode(
        ...
        Map<String, Object> result = new HashMap<>();
        try {
            String currentMode = tradeService.switchTradeMode(userId, mode);
            result.put("code", 200);
    ...
    /**
     * 模拟交易账户概览
     * GET /api/trade/paper-account
     */
    @GetMapping("/paper-account")
    public ResponseEntity<Map<String, Object>> paperAccount(
            @RequestHeader("X-User-Id") Long userId) {
        Map<String, Object> result = new HashMap<>();
        try {
            AccountOverview overview = tradeService.getPaperAccountOverview(userId);
            result.put("code", 200);
            result.put("message", "查询成功");
            result.put("data", overview);
            return ResponseEntity.ok(result);
