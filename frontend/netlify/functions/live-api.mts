...
async function fetchSectorRows() {
  ...
  for (const group of SECTOR_GROUPS) {
    const stocks = (await fetchSina(group.codes)) as AnyRecord[];
...
async function labState(userId: number) {
  return await blobStore().get(`ai-lab-state:${userId}`, { type: "json" }) || {
    generation: 0,
    iterationCount: 0,
    capital: 100000,
    intervalMinutes: 5,
...
async function analyze(code: string) {
  ...
  return {
    ...
    modelUsed: "Netlify Quant Sentiment Engine",
    modelAvailable: true,
    quantDecision: {
      signal: finalSignal,
      confidence: score,
...
async function route(req: Request) {
  ...
  if (method === "GET" && path === "/ai/lab/iterations") {
    return send(await blobStore().get(`ai-lab-iterations:${userIdFrom(req)}`, { type: "json" }) || []);
  }
  ...
  if (method === "GET" && path === "/ai/configs") return send(await blobStore().get("ai-configs", { type: "json" }) || [{ id: 1, name: "公网量化舆情引擎", provider: "netlify", modelName: "quant-sentiment", enabled: true, createTime: new Date().toISOString() }]);
