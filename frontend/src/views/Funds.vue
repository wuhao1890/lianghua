    ...
    <el-card shadow="hover" class="signal-card" style="margin-top: 16px">
      <template #header>
        <div class="card-header">
          ...
          <el-tag size="small" type="info" effect="plain">基于市场整体行情分析</el-tag>
        </div>
      </template>
      <SignalIndicator :signals="signals" stock-code="MARKET_INDEX" />
    </el-card>
  </div>
</template>
...
async function loadSignals() {
  try {
    const res = await getTradeSignal('MARKET_INDEX')
