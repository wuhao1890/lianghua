  ...
  <div class="market-hub">
    <div class="page-header">
      <div class="header-left">
        ...
        <span class="page-title">港股市场</span>
      </div>
    </div>
      ...
      <el-tab-pane label="行情列表" name="list">
        <el-card shadow="hover" class="market-card">
          ...
          >
            ...
            <el-table-column prop="currentPrice" label="现价/HKD" width="120" align="right">
              <template #default="{ row }">
                <span :style="{ color: getColor(row.changePercent), fontWeight: 600 }">
                  HK${{ formatPrice(row.currentPrice, 2) }}
                </span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="high" label="最高" width="110" align="right">
              <template #default="{ row }">
                <span>HK${{ formatPrice(row.high, 2) }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="low" label="最低" width="110" align="right">
              <template #default="{ row }">
                <span>HK${{ formatPrice(row.low, 2) }}</span>
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="大盘指数" name="indices">
        <div class="indices-section">
          <div class="section-header">
            <span class="section-title">港股大盘指数</span>
...
async function loadData() {
  ...
  try {
    ...
    if (keyword) {
      const res = await searchGlobal('HK', keyword)
      stockList.value = res.data?.data || []
      total.value = stockList.value.length
    } else {
      const res = await getGlobalStockList('HK', {
    ...
    ElMessage.error(e.message || '加载港股数据失败')
    stockList.value = []
    total.value = 0
  } finally {
    loading.value = false
...
async function loadIndices() {
  ...
  try {
    const res = await getGlobalIndices('HK')
