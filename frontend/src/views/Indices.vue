...
// 生成模拟指数K线数据
function generateMockKline(currentValue: number) {
  const dates: string[] = []
  const prices: number[][] = []
  const volumes: number[] = []
  const today = new Date()
  // 用当前值作为种子，确保每次刷新数据一致
  const seed = Math.round(currentValue)
  let price = currentValue * 0.9
  ...
  for (let i = 89; i >= 0; i--) {
    ...
    const change = (r - 0.45) * price * 0.018
    const open = price
    const close = price + change
    const high = Math.max(open, close) * (1 + r * 0.008)
    const low = Math.min(open, close) * (1 - (1 - r) * 0.008)
    const vol = Math.round((r * 0.5 + 0.5) * 50000000000)
...
function initChart(code: string) {
  ...
  const klineData = generateMockKline(index.current)
  const closes = klineData.prices.map(p => p[1])
  ...
  // 成交量颜色
  const volColors = klineData.prices.map(p =>
    p[1] >= p[0] ? '#f56c6c' : '#67c23a'
  )
  ...
  chart.setOption({
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        const d = k.axisValue
        const v = k.data
        const vol = klineData.volumes[k.dataIndex]
        const volStr = vol >= 100000000 ? (vol / 100000000).toFixed(2) + '亿' : (vol / 10000).toFixed(2) + '万'
        return `<div style="font-weight:600;margin-bottom:6px">${d}</div>
          <div>开: <b>${v[0].toFixed(2)}</b></div>
          <div>收: <b style="color:${v[1] >= v[0] ? '#f56c6c' : '#67c23a'}">${v[1].toFixed(2)}</b></div>
          <div>高: <b>${v[3].toFixed(2)}</b></div>
          <div>低: <b>${v[2].toFixed(2)}</b></div>
          <div>成交量: ${volStr}</div>`
      }
    },
    grid: [
      { left: '3%', right: '3%', top: '5%', height: '62%' },
    ...
    series: [
      ...
      },
      {
        name: '成交量',
        type: 'bar',
        data: klineData.prices.map((p, i) => ({
          value: klineData.volumes[i],
