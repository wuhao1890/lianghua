          ...
          <div class="account-info">
            ...
            <div class="info-row mode-row">
              ...
              <div class="mode-toggle">
                <el-tag :type="tradeMode === 'REAL' ? 'danger' : 'warning'" size="small" style="margin-right:8px">
                  {{ tradeMode === 'REAL' ? '实盘' : '模拟' }}
                </el-tag>
                <el-switch
                  :model-value="tradeMode === 'PAPER'"
                  active-text="模拟"
                  inactive-text="实盘"
            ...
            <div v-if="tradeMode === 'PAPER' && paperAccount" class="info-row">
              <span class="label">虚拟余额</span>
              <span class="value text-success">{{ formatMoney(paperAccount.balance) }}</span>
            </div>
          </div>
        </el-card>
...
import { getTradeMode, switchTradeMode, getPaperAccount } from '@/api/trade'
...
const tradeMode = ref('REAL')
const paperAccount = ref<any>(null)
...
async function loadTradeMode() {
  try {
    ...
    const data = res.data
    if (data.code === 200) {
      tradeMode.value = data.data?.mode || data.data || 'REAL'
      if (tradeMode.value === 'PAPER') {
        loadPaperAccount()
      }
    }
  } catch (e) {
...
async function handleModeSwitch(checked: boolean) {
  const newMode = checked ? 'PAPER' : 'REAL'
  try {
    const res = await switchTradeMode(newMode)
    if (res.data.code === 200) {
      tradeMode.value = newMode
      ElMessage.success(`已切换至${newMode === 'PAPER' ? '模拟交易' : '实盘交易'}模式`)
      if (newMode === 'PAPER') {
        loadPaperAccount()
      } else {
